...
input_loop:
    ...
    mov edx, 4                            ; Set edx to 4 for accessing elements (4 bytes per integer)
    mov ebx, array                        ; Base address of the array
    lea esi, [ebx + 16]                   ; End address of the array (5 integers * 4 bytes = 20 bytes)

reverse_loop:
    cmp ecx, 2                            ; If start index >= 2 (half of array), stop
    jge reverse_done

    ; Swap elements at array[ecx] and array[esi]
    mov eax, [ebx + ecx*4]                ; Load value from the start of the array
    mov ebx, [esi]                        ; Load value from the end of the array
    mov [ebx + ecx*4], ebx                ; Store the swapped value at the start of the array

    ; Adjust pointers
    inc ecx                                ; Increment the start index
    dec esi                                ; Decrement the end index
